...
Needle 3 support runs the vendored, unmodified Needle WebAssembly engine (`src/vendor/needle`) in a Web Worker and downloads the 35 MB `needle3.cact` weights from a pinned Hugging Face revision. The weights are checked against a pinned SHA-256 and kept in the browser's Cache Storage. It needs no WebGPU. Needle receives only the request text, never the current tasks or conversation, so it gets its own view of the tools: `add_task` asks it to keep the user's wording, and a Needle-only `record_finished_task` covers work the user already did. The app translates that call into `complete_task` for a matching open task, or `add_task` followed by `complete_task`. While a "Which task should I complete?" question is pending, Needle is offered only `complete_task` limited to the listed tasks. The deterministic guardrails still handle ambiguous, missing, and unsupported requests.

**Needle first** pairs Needle 3 with whichever larger model is loaded (Chrome's Gemini Nano, Gemma, or Bonsai). Needle plans short, direct requests in about a second. The app hands a request to the larger model instead when it is a proposal refinement or an open-ended clarification answer, longer than 25 words, planned with confidence below 0.6, planned with no calls, or completes a task that does not exist. Requests that the deterministic guardrails will decide anyway stay with Needle. Each trace records which model decided and why Needle was bypassed.
